	...
	for(var index = 0; index < section.children.length; index++){
		...
		if(string.getAttribute("target") == "Mobile" || string.getAttribute("target") == "IOS"){
			if(section.children[index].getElementsByTagName(language)[0].childNodes.length == 1){
				sectionStringsFile += '"'+string.getAttribute("id") +'" = "' + xmtToLocalizableString(string.getElementsByTagName(this.language)[0].childNodes[0].nodeValue) + '";\n';				
			}else{				
				...
				sectionStringsFile += '"'+string.getAttribute("id")+'_singular" = "' + xmtToLocalizableString(single)  + '";\n';
				sectionStringsFile += '"'+string.getAttribute("id")+'_plural" = "' + xmtToLocalizableString(plural)  + '";\n';	
			}
		}
	}
